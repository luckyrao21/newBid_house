...
exports.add = (request, response, next) => {
    ...
    request.body.productImage =
        "https://firebasestorage.googleapis.com/v0/b/productdb-eaa0c.appspot.com/o/" +
        request.file.filename +
        "?alt=media&token=abcddcba";
    Product.create(request.body)
        .then((result) => {
            return response.status(201).json({
                data: result,
                message: "Your Product will be display After Approved by Admin",
            });
        })
        .catch((err) => {
            return response
...
exports.delete = (request, response, next) => {
    ...
    Product.deleteOne({ _id: request.body.productId })
        .then((result) => {
            ...
            } else return response.status(201).json({ notDelete: "Not Deleted " });
        })
        .catch((err) => {
            return response
                .status(201)
...
exports.searchProduct = (request, response, next) => {
    ...
    Product.find({
        ...
        .then((result) => {
            if (result.length > 0) return response.status(201).json(result);
...
exports.approvedProductListBySeller = (request, response, next) => {
    Product.find({
            $and: [{ isApproved: true }, { creator: request.body.creatorId }],
        })
        .then((result) => {
            if (result.length > 0) return response.status(201).json(result);
            else
...
exports.cancelProductListBySeller = (request, response, next) => {
    Product.find({
            $and: [{ isApproved: false }, { creator: request.body.creatorId }],
        })
        .then((result) => {
            if (result.length > 0) {
                return response.status(201).json(result);
...
exports.cancelProductAuction = (request, response, next) => {
    Product.updateOne({ startTime: { $lt: Date.now() }, _id: request.body.productId }, {
        ...
        .then((result) => {
            ...
            else return response.status(201).json({ error: "Not Canceled.." });
        })
        .catch((err) => {
            console.log(err + "=============================================errrrr");
            return response
